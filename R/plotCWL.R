...
#' wf <- wf + s1 + s2
#' plotCWL(wf)
plotCWL <- function(cwl, output = "viz", layout = "tree", ...){
    if(is(cwl, "cwlWorkflow")){
        if(output == "viz"){
            plotViz(cwl)
        }else if(output == "mermaid"){
            plotMermaid(cwl, ...)
...
plotCP <- function(cwl, layout = "tree", ...){
    Inputs <- names(inputs(cwl))
    Outputs <- names(outputs(cwl))
    ...
    if(length(cwl@id) > 0){
        ...
        cmd <- cwl@baseCommand[[1]]
    }
    
    nodes <- c(Inputs, Outputs, cmd)
    if(length(nodes) > 26){
        nn <- length(nodes) - 26
        NodeID <- c(LETTERS, paste0(LETTERS[seq(nn)], seq(nn)))
    }else{
    ...
    shape <- rep(c("rectangle", "circle", "diamond"),
                 c(length(Inputs), length(Outputs), 1))
    ntable <- data.frame(id = NodeID, label = nodes, shape = shape)
    ntable <- data.frame(ntable, id_ext = ntable$id)
...
plotViz <- function(cwl){
    if(!file.exists(Sys.which("cwltool"))){
        cl <- basiliskStart(env_Rcwl)
        basiliskStop(cl)
    }
    cwlFile <- writeCWL(cwl)
    dot <- system2("cwltool", paste("--print-dot", cwlFile[1]), stdout = TRUE)
    grViz(dot)
}
